...
num_channel_input = 3
num_classes = 100
normalizedmean = (0.485, 0.456, 0.406)  #Cifar10:(0.4914, 0.4822, 0.4465)
normalizedstd = (0.229, 0.224, 0.225)  #Cifar10:(0.2023, 0.1994, 0.2010)
...
varmode = 'bm'
leakratio_list = [8,12]   #effective result:8  #pre-test:12
samplefreq_list = [10,15]   #effective result:10  #pre-test:15
trun_list= [0.02, 0.03] #effective result:0.02
significance = 0.05
dropfactor = 10
        ...
        for samplefreq in samplefreq_list:
            for trun in trun_list:
                ...
                if my_model == "preactresnet18" or my_model == "preactresnet34":
                    ...
                    lr = 1    #1
                '''
                temp_model = modeldic[my_model]
                optimizer = SSM(temp_model.parameters(), lr=lr, weight_decay=wd, momentum=momentum, dampening = dampening, testfreq=testfreq, var_mode=varmode, 
                                leak_ratio=leakratio, minN_stats=minstats, mode=keymode, samplefreq=samplefreq, significance=significance, drop_factor=dropfactor)
                total_parameter = sum(p.numel() for p in temp_model.parameters())
                ...
                for epoch in range(num_epochs):
                    ...
                    for i, (images, labels) in enumerate(trainloader):
                        ...
                        if (my_model == "mgnet128") or (my_model == "mgnet256"):
                            outputs = temp_model(0,images)   # We need additional 0 input for u in MgNet
                        else:
                    ...
                    if test_accuracy > max_test_accuarcy:
                        ...
                        best_parameter = temp_model.state_dict()
                        peak_epoch = epoch
                ...
                f.write("ssm100_lk{}_sf{}_tr{}".format(leakratio,samplefreq,sign_trun) + my_model +"_testacculist = {}\n".format(test_accuracy_list))
                f.write("ssm100_lk{}_sf{}_tr{}".format(leakratio,samplefreq,sign_trun) + my_model +"_lrlist = np.log10(np.array({}))\n".format(lr_list))
                f.write("ssm100_lk{}_sf{}_tr{}".format(leakratio,samplefreq,sign_trun) + my_model +"_statlist = {}\n".format(statistic_list))
